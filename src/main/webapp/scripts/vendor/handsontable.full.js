...
//        instance.rootElement.addClass(settings.className);
    ...
    if (height){
      ...
      if (typeof settings.mergeCells !== 'undefined' && !init) {
  ...
  this.setCellMetaObject = function (row, col, prop) {
    ...
    this.setCellMeta = function (row, col, key, val) {
        setCellMetaValue(row, col, key, val);
        Handsontable.hooks.run(instance, 'afterSetCellMeta', row, col, key, val);
    };

    /**
     * Sets cells meta values
     * @param {Array} metaValues array of meta value in format [{row: number, col: number, key: string, value: *}, ...]
     */
    this.setCellsMetaValues = function (metaValues) {
        for (var index = 0; index < metaValues.length; index++) {
            var metaValue = metaValues[index];
            setCellMetaValue(metaValue.row, metaValue.col, metaValue.key, metaValue.value);
        }
        Handsontable.hooks.run(instance, 'afterSetCellsMetaValues');
    };

    /**
     * Sets cell meta objects
     * @param cellMetaObjects array of cell meta object in format [{row: number, col: number, metaObject: {*}}, ...]
     */
    this.setCellsMetaObjects = function (cellMetaObjects){
        for(var arrayIndex in cellMetaObjects){
            var cellMetaObject = cellMetaObjects[arrayIndex];
            if (typeof cellMetaObject === 'object') {
                for (var prop in cellMetaObject.metaObject) {
                    setCellMetaValue(cellMetaObject.row, cellMetaObject.col, prop, cellMetaObject.metaObject[prop]);
                }
            }
        }
        Handsontable.hooks.run(instance, 'afterSetCellsMetaObjects');
    };

    /**
     * Sets cell meta value object "key" corresponding to params row, col
     * @param {Number} row
     * @param {Number} col
     * @param {String} key
     * @param {String} val
     *
     */
    function setCellMetaValue(row, col, key, val){
...
(function (Handsontable) {
  ...
  BaseEditor.prototype.isWaiting = function () {
    ...
    BaseEditor.prototype.getSimpleEditedCell = function () {
    ...
    BaseEditor.prototype.changeZIndex = function () {
        var editorSection = this.checkEditorSection()
        switch (editorSection) {
            case 'top':
                this.setZIndex(101);
...
//    this.TD = this.instance.getCell(this.row, this.col);
    ...
    var width = Handsontable.Dom.outerWidth(this.TD)  //$td.width()
      , maxWidth = this.instance.view.maximumVisibleElementWidth(cellLeftOffset) //10 is TEXTAREAs border and padding
      , height = Handsontable.Dom.outerHeight(this.TD)  //$td.outerHeight() - 4
...
(function (Handsontable) {
  ...
  Handsontable.editors.registerEditor('select', SelectEditor);
    ...
    /**
     * Created by vitalii.sergey on 18.08.2014.
     */
    (function (Handsontable) {
        ...
        ChosenSelectEditor.prototype.open = function () {
            this.TD = this.getSimpleEditedCell();
            var width = Handsontable.Dom.outerWidth(this.TD); //important - group layout reads together for better performance
            var height = Handsontable.Dom.outerHeight(this.TD);
    ...
    (function (Handsontable) {
        var SingleChosenSelectEditor = Handsontable.editors.ChosenSelectEditor.prototype.extend();

        SingleChosenSelectEditor.prototype.init = function () {
            this.select = document.createElement('SELECT');
            Handsontable.Dom.addClass(this.select, 'htChosenSelectEditor');
            Handsontable.Dom.addClass(this.select, 'chosen-select');
            this.select.style.display = 'none';
        ...
        SingleChosenSelectEditor.prototype.open = function () {
            this.TD = this.getSimpleEditedCell();
            var width = Handsontable.Dom.outerWidth(this.TD); //important - group layout reads together for better performance
            var height = Handsontable.Dom.outerHeight(this.TD);
            var rootOffset = Handsontable.Dom.offset(this.instance.rootElement);
            ...
            this.select.style.margin = '0px';
            this.select.style.display = '';
            $(this.select).chosen({disable_search: true});
            this.chosenContainer = $('.handsontable .chosen-container').get(0);
            this.chosenContainer.style.top = tdOffset.top - rootOffset.top + 'px';
            this.chosenContainer.style.left = tdOffset.left - rootOffset.left + 'px';
            this.chosenContainer.style.position = 'absolute';
            ...
            $(this.select).data("chosen").is_opened = true;
            $(this.select).data("chosen").results_show();
        };


        SingleChosenSelectEditor.prototype.focus = function () {
            $(this.select).data("chosen").activate_field();

        };


        SingleChosenSelectEditor.prototype.close = function () {
            $(this.select).chosen('destroy');
            this.select.style.display = 'none';
            this.instance.removeHook('beforeKeyDown', this.onBeforeKeyDown);
        };

        SingleChosenSelectEditor.prototype.onBeforeKeyDown = function (event) {
            var editor = this.getActiveEditor();
            switch (event.keyCode) {
                case Handsontable.helper.keyCode.ENTER:
                {
        ...
        Handsontable.editors.SingleChosenSelectEditor = SingleChosenSelectEditor;
        Handsontable.editors.registerEditor('singlechosenselect', SingleChosenSelectEditor);
    })(Handsontable);
(function (Handsontable) {
...
function HandsontableColumnSorting() {
  ...
  this.sort = function () {
    ...
    switch (colMeta.type) {
      case 'date':
        sortFunction = colMeta.sortFunction ? colMeta.sortFunction : dateSort;
        break;
      default:
...
//Handsontable.hooks.add('afterUpdateSettings', beforeInit);
...
WalkontableScroll.prototype.scrollToRenderedCell = function (TD) {
  ...
  if (cellHeight < workspaceHeight) {
    if (viewportScrollPosition.top + topCloneHeight - cellOffset.top > 1) {
      ...
      (function() {
          ...
          AbstractChosen = (function() {
              ...
              AbstractChosen.prototype.keyup_checker = function(evt) {
                  ...
                  switch (stroke) {
                      ...
                      case 13:
                          evt.preventDefault();
                          if (this.results_showing && !this.is_opened) {
                              return this.result_select(evt);
                          }
                          else
                          {
                              this.form_field_jq.trigger('chosen:on:enter');
              ...
              AbstractChosen.default_multiple_text = "Bitte auswählen";
          ...
          Chosen = (function(_super) {
              ...
              Chosen.prototype.set_up_html = function() {
                  ...
                  if (this.is_multiple) {
                      this.container.html('<ul class="chosen-choices"><li class="search-field"><input type="text" class="default handsontableInput" autocomplete="off" style="width:25px;" /></li></ul><div class="chosen-drop"><ul class="chosen-results" style="height: 88px"></ul></div>');
                  } else {
                      this.container.html('<a class="chosen-single chosen-default" tabindex="-1"><span>' + this.default_text + '</span><div><b></b></div></a><div class="chosen-drop"><div class="chosen-search"><input type="text" autocomplete="off" /></div><ul class="chosen-results" style="height: 88px"></ul></div>');
                  }
